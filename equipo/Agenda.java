...
public class Agenda {
	...
	public void cambiarCorreo(String nombre, String apellido, String nuevoCorreo) {
	    boolean encontrado = false;
	    // Primero se busca el contacto y al encontrarse se le cambia el correo
	    for (Contacto c : agenda) {
	        if (c.getNombre().equalsIgnoreCase(nombre) || 
	            c.getApellidos().equalsIgnoreCase(apellido)) {
	...
	//Método auxiliar para validar un apellido dentro de la lista de contactos
	public boolean validarApellidos(String apellidos) {
		for (Contacto c : agenda) {
			if (c.getApellidos().equalsIgnoreCase(apellidos.trim())){
				return true;
			}
		}
		return false;
	...
	//f) Método para eliminar un contacto de la agenda
		public void eliminarContacto(String correo, String alias) {
			...
			for (Contacto c : agenda) {
				...
				if (correo.equals(c.getCorreo()) && alias.equalsIgnoreCase(c.getAlias())) {
					contactoAEliminar = c;
					break;
				}
			}
			if (contactoAEliminar != null) {
				...
				agenda.remove(contactoAEliminar); //Java busca el objeto y lo elimina
				System.out.println("-- Contacto eliminado --");
			} else {
				System.out.println("-- Error: No se encontró ningún contacto con esos datos --");
			}
		}
	...
	//h) Método para consultar a una persona y obtener sus datos
		public void consultarPersona(String alias) {
		    boolean encontrado = false;
		...
		public void consultarPersona(String nombre, String apellidos) {
		    boolean encontrado = false;
		    // Se busca y se imprime el toString del contacto, el cual incluye
		    // el toString de persona y el toString de sus teléfonos
		    for (Contacto c : agenda) {
		        if (c.getNombre().equalsIgnoreCase(nombre.trim()) && 
		            c.getApellidos().equalsIgnoreCase(apellidos.trim())) {
		            System.out.println(c.toString());
		            encontrado = true;
		            break;
		        }
